	...
	const EachChip = newSkillsArray.map(skill => {
		...
		if (skill === 'React') {
			...
			bgColor = {
				...
				backgroundImage:
					'linear-gradient(90deg, #529949 25%, #412f1f 100%)'
			};
		} else if (skill.includes('Firebase')) {
			bgColor = {
				background: '#f5820b',
				backgroundImage:
					'linear-gradient(270deg, #f5820b 25%, #FFE32C 100%)'
			};
		} else if (skill === 'Python 3' || skill === 'Pygame') {
			bgColor = {
				backgroundColor: '#336c9c',
				backgroundImage:
					'linear-gradient(90deg, #336c9c 25%, #FFE32C 100%)'
			};
		} else if (skill === 'Postgresql') {
			bgColor = {
				backgroundColor: '#0064a5',
				backgroundImage:
					'linear-gradient(90deg, #0064a5 25%, #336791 100%)'
			};
		} else if (skill === 'Nginx') {
			bgColor = {
			...
			bgColor = {
				...
				backgroundImage:
					'linear-gradient(90deg, rgb(225, 84, 70) 25%, rgb(124, 28, 20) 100%)'
			};
		} else if (skill.includes('AWS')) {
			bgColor = {
		...
		return (
			<Card className={classes.card}>
				{onePortfolio.category && (
					<Typography
						component='p'
						style={{ padding: '0.5rem', fontWeight: 'bolder' }}
					>
						{onePortfolio.category}
					</Typography>
				)}
				...
				>
					...
					<CardContent>
						...
						{Array.isArray(onePortfolio.content) ? (
							onePortfolio.content.map((val, index) => (
								<Typography
									key={index}
									component='p'
